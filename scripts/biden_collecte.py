...
base_url = "https://polititweet.org/tweets?account=939091"
all_data = []
total_pages = 304

print(f"Début du scraping complet de {total_pages} pages...")
...
for page in range(1, total_pages + 1):
    ...
    try:
        ...
        for container in containers:
            ...
            tweet_id = f"biden_{page}_{len(all_data)}"
        ...
        print(f"Page {page}/{total_pages} terminée ({len(all_data)} tweets au total)")
        time.sleep(1)
...
df.to_csv('data/csv/biden/BIDEN_TWEETS.csv', index=False, encoding='utf-8')
